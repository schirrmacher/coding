...
- Named protocol sections. Use an ALL-CAPS *label line* — a bare or bold line ending in a colon ("Git Safety Protocol:", "CRITICAL RULES FOR EDITING:", "REQUIRED STEPS", "OUTPUT FORMAT (REQUIRED)") — before a numbered/bulleted list. Skimmable label the model can reference internally and the caller can grep for. Not a markdown heading: ALL CAPS belongs inline or in a label line, never in a `#`/`##` header. Headers stay Title Case so the two carry different signals.
